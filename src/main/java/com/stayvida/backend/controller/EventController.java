...
public class EventController {
    ...
    public Object createEvent(@RequestBody Map<String, Object> body) {
        try {
            ...
            String eventID = "E" + (int) (Math.random() * 900 + 100);
    ...
    public Object searchEvents(@RequestBody Map<String, Object> body) {
        try {
            ...
            String sql = """
                    ...
                    WHERE e.eventType = ?
                    AND e.guestCount >= ?
                    AND LOWER(SUBSTRING(h.destination, 1, 3)) = ?
            ...
            for (Map<String, Object> row : results) {
                ...
                String base64Image = (String) row.get("images");

                if (base64Image != null && !base64Image.isEmpty()) {
                    base64Image = base64Image.trim().replace("\"", "");
                }

                hotelData.put("image", base64Image);
    ...
    public Object getEventDetails(@RequestParam String eventID, @RequestParam Integer hotelID) {
        try {
            ...
            // Handle hotel images (Base64 already stored in DB)
            try {
                String imageStr = (String) row.get("hotel_images");
                List<String> images = new ArrayList<>();

                if (imageStr != null && !imageStr.isEmpty()) {
                    imageStr = imageStr.trim().replace("\"", "");
                    ...
                    try {
                        ...
                        // corrupted base64 → ignore
                    }
                }

                hotelData.put("images", images);
